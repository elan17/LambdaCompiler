...
module Lambda.Internals.LambdaTypes
    ( LambdaTerm(..)
...
toStringParams (Parametros (x : xs)) =
    x ++ " " ++ toStringParams (Parametros xs)

toString :: LambdaTerm -> String
toString (LambdaVariable x) = x
toString (LambdaStub     x) = x
toString (LambdaApplication x y) =
    "(" ++ toString x ++ " " ++ toString y ++ ")"
toString (LambdaFunction vars c) =
    "(λ" ++ toStringParams vars ++ "." ++ toString c ++ ")"
