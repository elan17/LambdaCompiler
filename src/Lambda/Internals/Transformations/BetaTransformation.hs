module Lambda.Internals.Transformations.BetaTransformation where
...
betaTransform t = helperBeta t H.empty
  where --helperBeta t m| trace ("term:" ++ show t ++ " bindings:"++show m) False = undefined
    helperBeta (LambdaVariable x) m =
        fromMaybe (LambdaVariable x) (H.lookup x m)
    ...
    helperBeta (LambdaApplication (LambdaFunction (Parametros (x : xs)) t1) t2) m
      ...
      where
        t1' = helperBeta t1 (H.insert x t2' (clearMap xs m))
        t2' = helperBeta t2 m

    helperBeta (LambdaApplication t1 t2) m = case t1' of
    ...
    helperBeta (LambdaFunction p@(Parametros l) t) m =
        LambdaFunction p $ helperBeta t (clearMap l m)

    clearMap params mapa = foldl (flip H.delete) mapa params
