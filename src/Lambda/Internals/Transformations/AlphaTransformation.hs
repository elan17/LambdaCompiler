module Lambda.Internals.Transformations.AlphaTransformation where
...
-- | Lista infinita de posibles nombres de variables para la transformación alpha
stringsGenerator :: [String]
stringsGenerator = helper "a"
  where
    helper item = item : helper (nextString item)
...
alphaTransform t = (\(x, _, _) -> x)
    (helperAlpha t M.empty M.empty stringsGenerator)
  where
    helperAlpha stub@(LambdaStub _) _ unbound l = (stub, unbound, l)
    ...
    helperAlpha (LambdaApplication x y) mapa unbound l =
      ...
      where
        (resul1, newUnbound1, newList1) = helperAlpha x mapa unbound l
    ...
    helperAlpha (LambdaFunction (Parametros xs) term) mapa unbound l =
      ...
      where
        (resul, newUnbound, newList) =
            helperAlpha term actualizaMapa unbound resto
        (valores, resto) = splitAt (length xs) l
        actualizaMapa =
            let nuevo = M.fromList $ zip xs valores in M.union nuevo mapa
