module Internals.Instances.Equality where
...
instance Eq LambdaTerm where
    (==) t1 t2 = helper (alphaTransform $ betaTransform t1) (alphaTransform $ betaTransform t2)
        where helper (LambdaStub x) (LambdaStub y) = x == y
